...
export default function CommunitiesPage() {
  ...
  const disciplines = ["All Disciplines", "Computer Science", "Physics", "Psychology", "Economics", "Biology", "Chemistry", "Mathematics"];
  ...
  const filterMentors = () => {
    let filtered = mentors.filter(mentor => {
      const matchesSearch = mentor.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
                           mentor.researchArea?.toLowerCase().includes(searchQuery.toLowerCase()) ||
                           mentor.institutionName?.toLowerCase().includes(searchQuery.toLowerCase());
      
      const matchesDiscipline = selectedDiscipline === "" || selectedDiscipline === "All Disciplines" || 
      ...
      const matchesPrice = mentor.hourlyRate && mentor.hourlyRate >= priceRange[0] && mentor.hourlyRate <= priceRange[1];
  ...
  const handleMentorClick = (mentorId: string) => {
    router.push(`/user/communities/${mentorId}`);
  };

  const handleMessageClick = (mentorId: string) => {
    router.push(`/user/communities/${mentorId}?tab=messages`);
  };
                  ...
                  <div className="flex items-center space-x-4 mb-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center space-x-1 mt-1">
                        ...
                        <span className="text-xs text-gray-500 truncate">{mentor.institutionName}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 mb-4">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-medium text-gray-900">
                        {mentor.hasCompleteProfile ? `${mentor.completedProjects} projects` : 'Profile in progress'}
                      </span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-gray-600">Response Time</span>
                      <span className="font-medium text-gray-900">
                        {mentor.hasCompleteProfile ? mentor.responseTime : 'TBD'}
                      </span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className={`font-medium ${
                        mentor.hasCompleteProfile 
                          ? (mentor.availability === 'Available' ? 'text-emerald-600' : 'text-red-600')
                          : 'text-gray-500'
                      }`}>
                        {mentor.hasCompleteProfile ? mentor.availability : 'Profile in progress'}
                      </span>
                    </div>
                  </div>
